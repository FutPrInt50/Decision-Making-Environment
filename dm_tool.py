...
# Globals are required to communicate data between the callback functions
# or nothing works otherwise
...
def get_HTML_Layout(ext_load=True, load_figures=False):
    ...
    html.Div(
        [
            html.Img(
                src="/assets/FutPrInt50_logo_transparent.png",
                style={"height": "17%", "width": "17%", "margin-right": "20px"},
            ),
            html.H1(
                children="Decision making environment"
            ),
        ],           
    ...
    html.Br(),
        ...
        html.Div(children=[
            ...
            html.Div(id='sd6',children=[
                 html.Div([
                    dcc.Graph(id='scatter-figure6',
                              figure= g_scatter_list[5] if load_figures else  go.Figure(),
                              style={'height': '31vw','width':  '31vw'}
                              )
                    ]),
    ...
    html.Br(),
    html.Div(id='copyright', children=['V. 0.55, Copyright Cranfield University']),
...
@g_app.callback(Output('scatter-figure6', 'figure'),
                Input('sd6-xaxis', 'value'),
                Input('sd6-yaxis', 'value'),
                Input('sd6-color', 'value'),
                Input('update_scatter','data'),
                prevent_initial_call=True)
def update_scatter6(x_label, y_label, c_label, dummy):
    global g_dataframe, g_scatter_list
    g_scatter_list[5] = generate_scatter(x_label, y_label, c_label, 5)
    
    return g_scatter_list[5]
...
def get_selected_data_parc(scatter_data1, scatter_data2, scatter_data3, 
    ...
    scatter6_points = [p['pointIndex'] for p in scatter_data6['points']] if scatter_data6 else []
...
def get_selected_data_scatters(restyleData, scatter_data1, scatter_data2, scatter_data3,
    ...
    g_scatter_selection_list[5] = scatter_data6['range'] if scatter_data6 and 'range' in scatter_data6.keys() else {}
...
def main_standalone(debug=True):
    ...
    @g_app.callback(Output('dropdown-parcoord', 'options'),
                    ...
                    Output('sd6-xaxis', 'options'),
                    Output('sd6-yaxis', 'options'),
                    Output('sd6-color', 'options'),
                    Output('sd6-xaxis', 'value'),
                    Output('sd6-yaxis', 'value'),
                    Output('sd6-color', 'value'),
